...
ggmanhattanPlot <- function(df,
  ...
  #add highlighted region to main df
  # if (is.null(highlight_int) == FALSE) {
  #   df <- df %>%
  #     mutate(highlight = case_when(pos >= highlight_int[1] & pos <= highlight_int[2] ~ "yes",
  #                                  pos < highlight_int[1] | pos > highlight_int[2] ~ "no"))
  # }
